...
treeList <- read_csv("hiton Tree_list/hilton_tree_list.csv")
treeList$YEAR <-  2005
treeList$VARIANT <- "NE"
treeList$LOCATION <- 922
...
--------------------------------------------------------------------------------------------------
  #here i specified lob as dominant species and 98 as site index
  
  # treeList$siteIndex <- NA
  # 
  # treeList <- treeList %>%
  #   mutate(siteIndex = ifelse(Species == "Lob", 98, siteIndex))
...
tree_1 <- treeList |>
  ...
  select( Plot_No, TreeNum, YEAR, SPP, dbh, Factor, VARIANT, LOCATION, add_tr,ht,SITE_INDEX) |>
  rename(
    PLOT_ID = Plot_No,
    TREE_ID = TreeNum,
    DBH = dbh,
...
saveWorkbook(wb, "todo_homen.xlsx", overwrite = TRUE)
